...
async function doApprove() {
  ...
  try {
    ...
    toast.success(action.value === 'APPROVE' ? '已通过' : '已驳回')
    target.value = null
    load()
...
<template>
  <div>
    <div class="page-head">
      <div>
        <h1 class="large-title">待审批</h1>
        <p class="page-subtitle">名下学生的请假申请与销假确认</p>
      </div>
    ...
    <div class="card">
      ...
      <EmptyState v-else-if="!list.length" icon="check" text="太棒了，暂无待办事项" />
          ...
          <div v-for="row in list" :key="row.id" class="group-row clickable pending-row" @click="router.push(`/leave/${row.id}`)">
            <div class="row-main">
              ...
              <div class="row-reason">
                ...
                <template v-if="row.status === 'CANCEL_PENDING' && row.cancelNote"> · 销假说明：{{ row.cancelNote }}</template>
              </div>
              <div class="row-meta">
                <span class="meta-item"><Icon name="calendar" :size="13" />{{ fmtTime(row.startTime) }} → {{ fmtTime(row.endTime) }}（{{ row.days }} 天）</span>
                <span class="meta-item"><Icon name="clock" :size="13" />提交于 {{ fmtTime(row.createTime) }}</span>
              </div>
            </div>
            <div class="row-actions" @click.stop>
              <button v-if="row.status === 'PENDING'" class="btn btn-sm btn-primary" type="button" @click="openApprove(row)">
                <Icon name="edit" :size="14" />审批
              </button>
              <button v-else-if="row.status === 'CANCEL_PENDING'" class="btn btn-sm btn-green" type="button" @click="cancelTarget = row">
                <Icon name="check" :size="14" />确认销假
              </button>
    ...
    <!-- 审批弹窗 -->
    <Modal :show="!!target" title="审批请假申请" width="520px" @close="target = null">
    ...
    <!-- 销假确认 -->
    <Modal :show="!!cancelTarget" title="确认销假" width="400px" @close="cancelTarget = null">
      <template v-if="cancelTarget">
        <p style="color:var(--text-2);font-size:14px;line-height:1.7">
          确认 <b style="color:var(--text-1)">{{ cancelTarget.studentName }}</b> 已返校销假？<br>
          <template v-if="cancelTarget.cancelNote">销假说明：{{ cancelTarget.cancelNote }}<br></template>
          确认后该请假单状态将变为「已完成」。
        </p>
      </template>
      <template #footer>
        <button class="btn btn-secondary" type="button" @click="cancelTarget = null">取消</button>
        <button class="btn btn-green" type="button" :disabled="confirming" @click="doCancelConfirm">确认销假</button>
      </template>
    </Modal>
  </div>
</template>
