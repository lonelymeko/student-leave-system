...
本模块为系统提供智能化与数据化两块增值能力。智能化侧基于 Spring AI 的 `ChatClient` 双供应商接入（**优先 OpenAI、回退 Anthropic**，`AI_PROVIDER` 可显式指定；model 与 base-url 均可环境变量自定义，兼容 DeepSeek/通义等 OpenAI 协议网关），落地三个贴合业务的 AI 能力：学生用自然语言一句话生成请假单草稿（智能填单）、辅导员审批时获取结合学生历史的风险评估与建议（审批建议）、全角色可用的请假制度问答浮窗（制度助手）；并设计了完整的**无 Key 优雅降级**机制，保证 AI 不可用时人工流程零受损。数据化侧为管理员提供统计看板：请假总量/各状态数量卡片、类型分布环形图、近 6 个月趋势折线图与状态分布，由后端 SQL 聚合 + 前端 ECharts 按需渲染。
...
| 依赖 | 版本 | 在本模块中的作用 |
|---|---|---|
| spring-ai-starter-model-openai | 1.0.0（spring-ai-bom 管理） | 自动装配 OpenAiChatModel，支持自定义 base-url 对接任意 OpenAI 兼容网关（优先供应商） |
| spring-ai-starter-model-anthropic | 1.0.0（spring-ai-bom 管理） | 自动装配 AnthropicChatModel，作为回退供应商 |
...
sequenceDiagram
    ...
    participant AM as ChatModel（按优先级选定：OpenAI → Anthropic）
    participant API as 模型服务（OpenAI 兼容 / Anthropic）
    ...
    AM->>API: HTTPS 调用所配模型（max-tokens 1024，5s连接/60s读超时）
    API-->>AM: 模型文本输出
    AM-->>CC: ChatResponse
    CC-->>B: content() 纯文本
...
配置在 `application.yml`：OpenAI 侧 `spring.ai.openai.*`（`OPENAI_API_KEY`/`OPENAI_BASE_URL`/`OPENAI_MODEL`），Anthropic 侧 `spring.ai.anthropic.*`（`ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL`/`ANTHROPIC_MODEL`），供应商策略 `app.ai.provider`（`AI_PROVIDER`，默认 auto=OpenAI 优先）。`AiService` 注入 `ObjectProvider<OpenAiChatModel>` 与 `ObjectProvider<AnthropicChatModel>`，`selectModel()` 按“显式指定 > auto 优先级”选取，首次调用时双检锁懒加载 `ChatClient.builder(model).build()`；未配任何真实 Key 或自动装配缺席都不会影响应用启动。出站 HTTP 由 `AiHttpConfig` 统一加 5s 连接/60s 读超时，自定义 endpoint 不可达时快速降级而非挂起。
...
| AI/模型配置 | `backend/src/main/resources/application.yml` | `spring.ai.openai.*` / `spring.ai.anthropic.*`：Key、base-url、model 全部环境变量化；`app.ai.provider` 供应商策略 |
| 供应商选择 | `backend/src/main/java/com/school/leave/ai/AiService.java` | `selectModel()`：显式指定 > auto（OpenAI 优先→Anthropic 回退） |
| 出站超时 | `backend/src/main/java/com/school/leave/config/AiHttpConfig.java` | RestClient 5s 连接/60s 读超时，防 endpoint 不可达挂起 |
