...
| **权限 RBAC(2)** | sys_role 角色、sys_user_role 用户角色 | 角色多对多，支持一人多角色/切换角色 |
...
> **规范化落地**：把原 sys_user 宽表按第三范式拆成 `sys_user 账号` + `student/teacher 身份档案`（消除学号/班级对非学生行恒空的稀疏列），并抽出 `college/major/class_info` 组织层级（消除 class_name 的传递依赖）。请假/审批表仍以 sys_user.id 为业务主体键，VO 用 JOIN 拼出姓名/班级，故**接口契约不变、前端零改动**。
...
### 4.1 物理模型要点（详见 §1 总览图 + schema.sql DDL）
